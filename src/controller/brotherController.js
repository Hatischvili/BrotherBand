  ...
  try {
    if (userGetter.brothers.includes(caughtId)) {
      const brotherInfo = {
        ...
        Age: moment.duration(moment().diff(moment(userCaught.birthdate)))._data[
          "years"
        ],
...
const postSendBBRequest = async (req, res) => {
  ...
  if (
    userRecipient.brothers.includes(senderId) ||
    userSender.brothers.includes(recipientId)
  ) {
...
const postSendMessage = async (req, res) => {
  ...
  try {
    if (userSender.brothers.includes(recipientId)) {
      ...
      res
        .status(406)
        .json("You cannot send messages to users whom are not your brothers");
    }
  } catch (ERR) {
    res.status(500).json({ Message: ERR.message });
  }
